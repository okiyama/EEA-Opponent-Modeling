...
class MinimaxPlayer(Konane, Player):
    ...
    def getMove(self, board):
        ...
        if self.ABPrune:
            self.alphaBetaMinimax(node, -100000, 100000)
        else:
            self.minimax(node)
    ...
    def staticEval(self, node):
        ...
        with the given node.
        """
        return self.countSymbol(node.state, node.player) - self.countSymbol(node.state, self.opponent(node.player))
        #return len(self.generateMoves(node.state, node.player))
