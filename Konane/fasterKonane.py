...
def main():
	game = Konane(6)
	print "Testing 50 games v. Random"
	# NOTE: We seed the random number generator for repeatability
	random.seed(42)
	wins = 0
	for i in xrange(50):
		result = game.playOneGame(RandomPlayer(6), minimax.MinimaxPlayer(6, 4), False)
		if result == 'W':
			wins += 1
	print "Done ... we won " + str(float(wins)/float(50)) + " Percent of the time"

if __name__ == '__main__':
	#main()
	Konane(6).playOneGame(RandomPlayer(6), minimax.MinimaxPlayer(6, 4), True)
